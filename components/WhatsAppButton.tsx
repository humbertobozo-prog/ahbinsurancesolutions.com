...
export const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({ ariaLabel, language }) => {
    ...
    return (
        <a
            ...
            title={ctaLabel}
            className="group fixed bottom-20 md:bottom-6 right-6 bg-[#25D366] text-white h-14 md:h-16 px-4 md:px-5 rounded-full flex items-center gap-2.5 shadow-xl hover:bg-[#20bd5a] transform hover:scale-105 transition-all duration-300 z-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#25D366]"
        >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 md:h-8 md:w-8 shrink-0" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            ...
            <span className="hidden sm:inline font-bold text-xs md:text-sm tracking-wide whitespace-nowrap">
                {ctaLabel}
            </span>
        </a>
    );
